    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            salvarDadosBtn.addEventListener('click', function() {
                ...
                const dadosAPI = neocredit?.dados_api; // Dados brutos da API
                ...
                // Pega da API: dados_api.campos.status
                const statusCampos = dadosAPI?.campos?.status ||
                    dadosAPI?.status ||
                    dadosFormatados.status ||
                    'CONSULTADO';

                console.log('Status dos campos:', statusCampos); // Debug
                ...
                formData.append('razao_social', dadosFormatados.razao_social || '');
                ...
                formData.append('status', statusCampos); // ← AGORA USA O STATUS DOS CAMPOS

                // Adicionar códigos do Firebird
                formData.append('codic_bm', firebird?.['BARRA MANSA'] || 'NÃO CADASTRADO');
                formData.append('codic_bot', firebird?.['BOTUCATU'] || 'NÃO CADASTRADO');
                formData.append('codic_ls', firebird?.['LINS'] || 'NÃO CADASTRADO');
                formData.append('codic_rp', firebird?.['RIO PRETO'] || 'NÃO CADASTRADO');
                formData.append('codic_vt_rnd', firebird?.['VOTUPORANGA'] || 'NÃO CADASTRADO');
                ...
                fetch('limite_cred_save.php', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // Mostrar qual status foi salvo
                            console.log('Status salvo:', data.status_salvo);

                            if (data.situ === 'A') {
                                console.log('Registro ativo criado com código:', data.codigo);
                            }
                        } else {
