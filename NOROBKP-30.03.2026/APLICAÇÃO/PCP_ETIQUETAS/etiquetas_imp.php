...
<head>
    ...
    <title>Impressão de Etiquetas - NOROAÇO</title>
...
<body>
    ...
    <div class="loading-screen" id="loadingScreen">
        ...
        <div class="loading-details" id="loadingDetails">
            <i class="fas fa-cog"></i> Preparando <?php echo count($listaOps); ?> OPs para impressão...
        </div>
                ...
                <div class="corpo-etiqueta">
                    ...
                    <div class="codes-section">
                        ...
                        <div class="side-card" data-type="produto">
                            <div class="side-card-title"><i class="fas fa-cube"></i> PRODUTO</div>
    ...
    <script>
        ...
        /**
         * Função principal para cancelar impressão e fechar a guia
         */
        ...
        /**
         * Função para fechar a guia atual e voltar para a página anterior
         */
        function fecharGuia() {
            console.log('🔒 Fechando guia...');
            
            // Verificar se a página foi aberta por window.open()
            if (window.opener && !window.opener.closed) {
                try {
                    ...
                    // Fallback: se não fechar em 500ms, redirecionar
                    setTimeout(function() {
                        window.location.href = 'etiquetas.php';
                    }, 500);
                } catch(e) {
                ...
                // Se não tiver opener, redirecionar para a página principal
                window.location.href = 'etiquetas.php';
            }
        }
        
        /**
         * Função para redirecionar (fallback)
         */
        ...
        function updateLoadingProgress() {
            ...
            if (loadingDetails) {
                ...
                loadingDetails.innerHTML = `
                    ...
                    <small style="opacity: 0.8;">${barcodesLoaded} de ${totalBarcodes} códigos processados</small>
                `;
            }
        }
        ...
        // Evento quando a página é totalmente carregada
        window.onload = function() {
            console.log('🚀 Página de impressão carregada!');
            
            // Verificar se tem opener
            if (window.opener) {
                console.log('📌 Página aberta como popup de:', window.opener.location.pathname);
            } else {
                console.log('📌 Página aberta diretamente');
            }
            ...
            if (loadingDetails) {
                loadingDetails.innerHTML = `
                    <i class="fas fa-spinner fa-pulse"></i> Carregando códigos de barras...<br>
                    <small style="opacity: 0.8;">0 de ${totalBarcodes} códigos processados</small>
                `;
            }
            ...
            // Se não houver códigos de barras, iniciar impressão imediatamente
            if (totalBarcodes === 0) {
                console.log('⚠️ Nenhum código de barras para carregar');
                allBarcodesLoaded = true;
                triggerAutoPrint();
            }
        ...
        window.addEventListener('beforeprint', function() {
            ...
            const totalEtiquetas = document.querySelectorAll('.etiqueta-container').length;
            console.log(`📄 Total de etiquetas a imprimir: ${totalEtiquetas}`);
        ...
        window.addEventListener('afterprint', function() {
            console.log('✅ Diálogo de impressão fechado');
            
            // Se a impressão foi concluída ou cancelada, fechar a guia
            setTimeout(function() {
                fecharGuia();
            }, 1000);
        });

        // Capturar tecla ESC para cancelar
        ...
        // Capturar quando a página ganha/lose foco
        window.addEventListener('focus', function() {
            console.log('✨ Guia ganhou foco');
        });
        
        window.addEventListener('blur', function() {
            console.log('💤 Guia perdeu foco');
        });
        
        // Prevenir que o usuário feche a página sem querer
        window.addEventListener('beforeunload', function(e) {
            // Se a impressão não foi iniciada, não mostrar confirmação
            if (!autoPrintTriggered) {
                console.log('👋 Guia fechada pelo usuário antes da impressão');
                return;
            }
            
            // Se já imprimiu, fechar normalmente
            if (printDialogOpened) {
                console.log('👋 Guia fechada após impressão');
                return;
            }
        ...
        // Adicionar evento para o botão de cancelar via JavaScript
        document.addEventListener('DOMContentLoaded', function() {
            const btnCancelar = document.getElementById('btnCancelar');
            if (btnCancelar) {
                btnCancelar.addEventListener('click', cancelarImpressao);
            }
        });
        
        // Impedir clique duplo no botão de cancelar
        let cancelando = false;
        window.cancelarImpressao = function() {
