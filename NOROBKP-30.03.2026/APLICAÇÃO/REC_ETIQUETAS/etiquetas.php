...
// Controle de sessão para detectar refresh
if (!isset($_SESSION['initialized'])) {
    // Primeira carga da página - inicializa sessão
...
// Função para buscar fornecedores de uma nota
...
// Função para buscar dados da nota
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Adicionar nota com fornecedor selecionado
                    ...
                    foreach ($dadosNota as $dados) {
                        ...
                        if (!$itemExistente) {
                            $novoItem = [
                                ...
                                'grupo_produto' => $dados['grupo_produto'],
    ...
    if (isset($_POST['cancelar_busca'])) {
        unset($_SESSION['nota_temp']);
        unset($_SESSION['fornecedores_temp']);
        $_SESSION['mensagem'] = "Busca cancelada!";
...
th {
    text-align: center !important; /* Centraliza todo o conteúdo do cabeçalho */
}
...
.cabecalho-centralizado th {
    text-align: center !important;
        ...
        /* BOTÃO LIMPAR TUDO SEMPRE VISÍVEL */
        .botao-limpar-sempre {
...
<body>
    <div class="container-principal">
        ...
        <div class="container-tabela">
            <div class="titulo-tabela">
                <i class="fas fa-ticket" style="color: #fdb525; margin-right: 10px;"></i> Sistema de Etiquetas - Recebimento
            </div>

            <!-- FORMULÁRIO DE ADIÇÃO - LADO A LADO -->
            <div class="formulario-centralizado">
                <form method="POST" class="formulario-lado-a-lado" id="form-nota">
                    ...
                    <input type="text"
                        ...
                        autofocus
                        value="<?php 
                            echo isset($_SESSION['nota_temp']) ? htmlspecialchars($_SESSION['nota_temp']) : '';
                        ?>"
                        id="campo-nota-input">
                    ...
                    <?php if (isset($_SESSION['nota_temp']) && isset($_SESSION['fornecedores_temp'])): ?>
                        <select name="fornecedor" class="campo-fornecedor" id="campo-fornecedor-select">
                        ...
                        <!-- BOTÃO CANCELAR -->
                        <button type="submit" name="cancelar_busca" class="btn-cancelar">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // DETECTAR REFRESH (CTRL+R OU BOTÃO)
            if (performance.navigation && performance.navigation.type === 1) {
                // TYPE 1 = PAGE_RELOAD
                console.log('Página recarregada detectada - limpando cache...');
                
                // Limpar campos do formulário
                const notaInput = document.getElementById('campo-nota-input');
                if (notaInput) {
                    notaInput.value = '';
                    notaInput.focus();
                }
                
                const fornecedorSelect = document.getElementById('campo-fornecedor-select');
                if (fornecedorSelect) {
                    fornecedorSelect.selectedIndex = 0;
                }
                
                // Limpar localStorage temporário
                localStorage.removeItem('nota_temp_data');
                localStorage.removeItem('form_data');
                
                // Forçar reload para limpar qualquer cache
                if (window.location.search.indexOf('reload=') === -1) {
                    window.location.search = '?reload=' + new Date().getTime();
                }
            }
            
            // DETECTAR SE A PÁGINA VEIO DO CACHE (BACK/FORWARD)
            ...
            window.onbeforeunload = function() {
                // Não fazer nada especial, só deixar a sessão ser destruída
            };
            
            // CAPTURAR TECLAS CTRL+R e F5
            document.addEventListener('keydown', function(e) {
                if ((e.ctrlKey && e.key === 'r') || e.key === 'F5') {
                    console.log('Refresh via teclado detectado - preparando limpeza...');
                    
                    // Adicionar um parâmetro para detectar na próxima carga
                    sessionStorage.setItem('force_clear', 'true');
                }
            });
            
            // VERIFICAR SE DEVEMOS LIMPAR
            if (sessionStorage.getItem('force_clear') === 'true') {
            ...
            // LIMPAR CAMPO APÓS ADICIONAR NOTA COM SUCESSO
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                if (e.submitter && e.submitter.name === 'cancelar_busca') {
                    setTimeout(function() {
                        if (campoNota) {
                            campoNota.value = '';
                            campoNota.focus();
            ...
            // IMPEDIR QUE O NAVEGADOR MANTENHA DADOS DO FORMULÁRIO
            if (window.history.replaceState) {
                window.history.replaceState(null, null, window.location.pathname);
            }
            
            // LIMPAR FORMULÁRIO ANTES DE SAIR DA PÁGINA
            window.addEventListener('beforeunload', function() {
                // Limpa apenas os campos temporários de busca
                const form = document.getElementById('form-nota');
                if (form) {
                    const notaInput = form.querySelector('[name="nota"]');
                    const fornecedorSelect = form.querySelector('[name="fornecedor"]');
                    
                    if (notaInput) {
                        // Só limpa se não houver nota_temp na sessão
                        if (!<?php echo isset($_SESSION['nota_temp']) ? 'true' : 'false'; ?>) {
                            notaInput.value = '';
                        }
                    }
            ...
            // ADICIONAR UM CONTADOR DE TEMPO PARA SESSÃO
            let sessionStartTime = Date.now();
            let sessionWarningTime = 30 * 60 * 1000; // 30 minutos
            
            setInterval(function() {
                let elapsedTime = Date.now() - sessionStartTime;
                if (elapsedTime > sessionWarningTime) {
                    console.log('Sessão muito longa - sugerindo refresh...');
                    // Você pode adicionar um aviso visual aqui se quiser
                }
            }, 60000); // Verifica a cada minuto
            
            // LOG PARA DEBUG
            console.log('Sistema carregado. Modo: ' + (performance.navigation.type === 1 ? 'RELOAD' : 'NORMAL'));
            console.log('Itens na lista: ' + <?php echo $totalItens; ?>);
        });
        
        // FUNÇÃO PARA FORÇAR LIMPEZA COMPLETA (PARA TESTES)
        function forceClean() {
            if (confirm('Isso irá limpar TUDO e recarregar a página. Continuar?')) {
                // Limpar tudo
                sessionStorage.clear();
                localStorage.clear();
                
                // Adicionar flag para limpar na próxima carga
                sessionStorage.setItem('force_full_clean', 'true');
        ...
        // FUNÇÃO PARA TESTAR O COMPORTAMENTO DE REFRESH
        function testRefresh() {
            alert('Testando refresh...\n\nAgora pressione Ctrl+R ou F5 para ver a página limpar completamente.');
            console.log('Teste de refresh iniciado');
        }
    </script>
</body>
</html>